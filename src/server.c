...
int beaker_run(const char *ip, int port) {
  ...
  while (true) {
    ...
    if (first_line_end == NULL) {
      fprintf(stderr, "[ERROR] Invalid HTTP request: No CRLF found.\n");
    ...
    if (request_line_len >= sizeof(request_line)) {
      fprintf(stderr, "[ERROR] Request line too long.\n");
      const char *bad_request =
          "HTTP/1.1 400 Bad Request\r\nContent-Length: 0\r\n\r\n";
      send(new_socket, bad_request, strlen(bad_request), 0);
      close(new_socket);
    ...
    if (requested_path == NULL) {
      fprintf(stderr,
              "Could not parse request path. Sending 400 Bad Request.\n");
      const char *bad_request =
          "HTTP/1.1 400 Bad Request\r\nContent-Length: 0\r\n\r\n";
      send(new_socket, bad_request, strlen(bad_request), 0);
      close(new_socket);
    ...
    if (!handled) {
      fprintf(stderr,
              "No handler or static file found for path '%s'. Sending 404 Not "
              "Found.\n",
              requested_path);
