...
static ContextVar *find_context_var(TemplateContext *ctx, const char *key) {
  if (ctx == NULL || key == NULL) {
    fprintf(stderr, "[ERROR] find_context_var: Invalid NULL input (ctx or key).\n");
    return NULL;
  }
...
static void free_string_2d_array_var(ContextVar *var) {
  ...
  for (int i = 0; i < var->value.string_2d_array_data.outer_count;
       i++) {
    if (var->value.string_2d_array_data.values[i] !=
        NULL) {
      for (int j = 0;
           j < var->value.string_2d_array_data.inner_counts[i]; j++) {
        if (var->value.string_2d_array_data.values[i][j] !=
            NULL) {
...
void context_set(TemplateContext *ctx, const char *key, const char *value) {
  if (ctx == NULL || key == NULL || value == NULL) {
    fprintf(stderr, "[ERROR] context_set: Invalid NULL input (ctx, key, or value).\n");
    return;
  }
  ...
  if (ctx->count < MAX_CONTEXT_VARS) {
    ...
    fprintf(stderr, "[WARNING] context_set: TemplateContext is full. Cannot add key '%s'.\n",
            key);
  }
}
...
void context_set_string_array(TemplateContext *ctx, const char *key,
  ...
  if (ctx == NULL || key == NULL || values == NULL) {
    fprintf(stderr, "[ERROR] context_set_string_array: Invalid NULL input (ctx, key, or values).\n");
    return;
  }
  if (count < 0 || count > MAX_OUTER_ARRAY_ITEMS) {
    fprintf(stderr,
            "[ERROR] context_set_string_array: Invalid count %d for string array context '%s'. Max %d allowed.\n",
  ...
  if (var != NULL) {
    ...
    if (ctx->count < MAX_CONTEXT_VARS) {
      ...
      fprintf(stderr,
              "[WARNING] context_set_string_array: TemplateContext is full. Cannot add string array key '%s'.\n",
              key);
      return;
    }
  }
  ...
  if (var->value.string_array_data.values == NULL) {
    ...
    fprintf(stderr, "[ERROR] context_set_string_array: Allocation failed for values for key '%s'.\n", key);
    return;
  }
  ...
  for (int i = 0; i < count; i++) {
    if (values[i] == NULL) {
      fprintf(stderr, "[ERROR] context_set_string_array: NULL value at index %d for key '%s'. Skipping.\n", i, key);
      var->value.string_array_data.values[i] = NULL; 
      continue;
    }
    ...
    if (var->value.string_array_data.values[i] == NULL) {
      ...
      fprintf(stderr,
              "[ERROR] context_set_string_array: Failed to duplicate value for item %d of key '%s'.\n",
              i, key);
...
void context_set_array_of_arrays(TemplateContext *ctx, const char *key,
  ...
  if (ctx == NULL || key == NULL || values_2d == NULL || inner_counts == NULL) {
    fprintf(stderr, "[ERROR] context_set_array_of_arrays: Invalid NULL input (ctx, key, values_2d, or inner_counts).\n");
    return;
  }
  if (outer_count < 0 || outer_count > MAX_OUTER_ARRAY_ITEMS) {
    fprintf(stderr,
            "[ERROR] context_set_array_of_arrays: Invalid outer count %d for array of arrays context '%s'. Max %d allowed.\n",
  ...
  if (var != NULL) {
    ...
    if (ctx->count < MAX_CONTEXT_VARS) {
      ...
      fprintf(stderr,
              "[WARNING] context_set_array_of_arrays: TemplateContext is full. Cannot add array of arrays key '%s'.\n",
              key);
      return;
    }
  }
  ...
  if (var->value.string_2d_array_data.values == NULL ||
    ...
    fprintf(stderr,
            "[ERROR] context_set_array_of_arrays: Allocation failed for key '%s'.\n",
            key);
    free(var->value.string_2d_array_data.values); 
    free(var->value.string_2d_array_data.inner_counts);
    var->value.string_2d_array_data.values = NULL;
  ...
  for (int i = 0; i < outer_count; i++) {
    ...
    if (current_inner_count < 0 ||
      ...
      fprintf(stderr,
              "[ERROR] context_set_array_of_arrays: Invalid inner count %d for item %d in 2D array '%s'. Max %d allowed.\n",
              current_inner_count, i, key, MAX_INNER_ARRAY_ITEMS);

      for (int k = 0; k < i; k++) {
        if (var->value.string_2d_array_data.values[k] !=
            NULL) {
          for (int l = 0;
               l < var->value.string_2d_array_data.inner_counts[k];
               l++) {
    ...
    if (var->value.string_2d_array_data.values[i] == NULL) {
      ...
      fprintf(stderr,
              "[ERROR] context_set_array_of_arrays: Allocation failed for inner array %d of key '%s'.\n",
              i, key);
    ...
    var->value.string_2d_array_data.inner_counts[i] =
        current_inner_count;

    for (int j = 0; j < current_inner_count; j++) {
      if (values_2d[i][j] == NULL) {
        fprintf(stderr, "[ERROR] context_set_array_of_arrays: NULL value at index [%d][%d] for key '%s'. Skipping.\n", i, j, key);
        var->value.string_2d_array_data.values[i][j] = NULL;
        continue;
      }
      var->value.string_2d_array_data.values[i][j] =
          strdup(values_2d[i][j]);
      if (var->value.string_2d_array_data.values[i][j] ==
          NULL) {
        perror("Failed to duplicate string for inner array context");
        fprintf(stderr,
                "[ERROR] context_set_array_of_arrays: Failed to duplicate value for item [%d][%d] of key '%s'.\n",
                i, j, key);
        ...
        for (int k = 0; k <= i; k++) {
          if (var->value.string_2d_array_data.values[k] !=
            ...
            for (int l = 0;
                 ...
                 (k == i
                      ? j
                      : var->value.string_2d_array_data.inner_counts[k]);
                 l++) {
              if (var->value.string_2d_array_data.values[k][l] != NULL) {
                free(var->value.string_2d_array_data.values[k][l]);
              }
...
static void append_to_buffer(char **buffer, size_t *current_len,
  ...
  if (str_to_add == NULL) {
    fprintf(stderr, "[WARNING] append_to_buffer: Attempted to append NULL string. Skipping.\n");
    return;
  }
...
static char *parse_indexed_tag(const char *tag_content, int *index_val) {
  *index_val = -1; 
  ...
  if (close_bracket == NULL) {
    fprintf(stderr,
            "[ERROR] parse_indexed_tag: Unclosed bracket in tag '%s'. Returning raw tag content.\n",
            tag_content);
...
static char *render_template_segment(const char *template_segment,
  ...
  if (rendered_buffer == NULL) {
    ...
    fprintf(stderr,
            "[ERROR] render_template_segment: Failed to allocate initial %zu bytes for rendered_buffer.\n",
            initial_max_len);
    return NULL;
  }
  rendered_buffer[0] = '\0'; 
  ...
  while ((start_tag = strstr(current_pos, "{{")) != NULL) {
    ...
    if (text_len > 0) {
      ...
      if (text_before_tag == NULL) {
        ...
        fprintf(stderr, "[ERROR] render_template_segment: Allocation failed for text_before_tag.\n");
    ...
    if (end_tag == NULL) {
      fprintf(stderr, "[ERROR] render_template_segment: Unclosed '{{' tag. Appending remaining template content as-is.\n");
    ...
    if (tag_content_raw == NULL) {
      ...
      fprintf(stderr, "[ERROR] render_template_segment: Allocation failed for tag_content_raw.\n");
      free(rendered_buffer);
      return NULL;
    }
    ...
    if (strncmp(trimmed_tag_content, "for ", 4) == 0) {
      ...
      if (sscanf(trimmed_tag_content, "for %s in %s", loop_var, list_var) == 2) {
        ...
        if (list_ctx_var != NULL) {
          ...
          if (loop_end_tag == NULL) {
            fprintf(stderr, "[ERROR] render_template_segment: '{{for}}' without matching '{{endfor}}'. Appending loop tag as-is.\n");
            append_to_buffer(&rendered_buffer, &current_len, &max_len, trimmed_tag_content);
          } else {
            ...
            if (loop_inner_template == NULL) {
              ...
              fprintf(stderr, "[ERROR] render_template_segment: Allocation failed for loop_inner_template.\n");
              free(rendered_buffer);
              free(tag_content_raw);
              return NULL;
            }
            ...
            if (list_ctx_var->type == CONTEXT_TYPE_STRING_2D_ARRAY) { 
              for (int i = 0; i < list_ctx_var->value.string_2d_array_data.outer_count; i++) {
                ...
                char *rendered_loop_item = render_template_segment(loop_inner_template, &loop_ctx);
                if (rendered_loop_item) {
                  append_to_buffer(&rendered_buffer, &current_len, &max_len, rendered_loop_item);
                  free(rendered_loop_item);
                }
                free_context(&loop_ctx); 
              }
            } else if (list_ctx_var->type == CONTEXT_TYPE_STRING_ARRAY) {
              for (int i = 0; i < list_ctx_var->value.string_array_data.count; i++) { 
                TemplateContext loop_ctx = new_context(); 
              ...
              fprintf(stderr,
                      "[ERROR] render_template_segment: List variable '%s' (type %d) is not an iterable array type for 'for' loop. Appending loop tag as-is.\n",
          ...
          fprintf(stderr,
                  "[ERROR] render_template_segment: List variable '%s' not found for 'for' loop. Appending loop tag as-is.\n",
                  list_var);
          append_to_buffer(&rendered_buffer, &current_len, &max_len, trimmed_tag_content);
        }
      } else {
        fprintf(stderr,
                "[ERROR] render_template_segment: Malformed 'for' loop tag: '%s'. Expected 'for var in list'. Appending loop tag as-is.\n",
                trimmed_tag_content);
        append_to_buffer(&rendered_buffer, &current_len, &max_len, trimmed_tag_content);
      }
    }
      ...
      fprintf(stderr, "[WARNING] render_template_segment: '{{endfor}}' without matching '{{for}}'. Appending endfor tag as-is.\n");
      append_to_buffer(&rendered_buffer, &current_len, &max_len, trimmed_tag_content);
    }
      ...
      if (*filename_start == '"') { 
        ...
        if (filename_end != NULL) {
          ...
          if (included_filename == NULL) {
            ...
            fprintf(stderr, "[ERROR] render_template_segment: Allocation failed for included_filename.\n");
            free(rendered_buffer);
            free(tag_content_raw);
            return NULL;
          }
          ...
          if (included_html) {
            append_to_buffer(&rendered_buffer, &current_len, &max_len, included_html);
            free(included_html);
          } else {
            fprintf(stderr, "[WARNING] render_template_segment: Failed to render included template '%s'.\n", included_filename);

            append_to_buffer(&rendered_buffer, &current_len, &max_len, "<!-- Failed to include ");
            append_to_buffer(&rendered_buffer, &current_len, &max_len, included_filename);
          ...
          fprintf(stderr,
                  "[ERROR] render_template_segment: Malformed include tag '%s'. Missing closing quote.\n",
                  trimmed_tag_content);
        ...
        fprintf(stderr,
                "[ERROR] render_template_segment: Malformed include tag '%s'. Expected quoted filename.\n",
                trimmed_tag_content);
      ...
      char *safe_flag_pos_in_processing = strstr(processing_tag_content, "|safe");
      ...
      if (key_name == NULL) {
        fprintf(stderr, "[ERROR] render_template_segment: parse_indexed_tag failed for '%s'.\n", trimmed_tag_content);
        free(rendered_buffer);
        free(tag_content_raw);
        return NULL;
      }
      ...
      if (var != NULL) {
        if (index_val == -1) { 
          if (var->type == CONTEXT_TYPE_STRING) {
            ...
            fprintf(stderr,
                    "[WARNING] render_template_segment: Variable '%s' is not a simple string (type %d). Not rendering as simple string.\n",
                    key_name, var->type);
          ...
          if (var->type == CONTEXT_TYPE_STRING_ARRAY) {
            if (index_val >= 0 &&
              ...
              fprintf(stderr,
                      "[ERROR] render_template_segment: Index %d out of bounds for '%s' (count %d). Appending tag as-is.\n",
            ...
            fprintf(stderr,
                    "[ERROR] render_template_segment: Variable '%s' (type %d) is not a string array for indexed access. Appending tag as-is.\n",
                    key_name, var->type);
        ...
        fprintf(stderr,
                "[WARNING] render_template_segment: Key '%s' not found in context. Appending tag as-is.\n",
                key_name);
      ...
      if (value_to_append != NULL) {
        if (is_safe) {
          append_to_buffer(&rendered_buffer, &current_len, &max_len, value_to_append);
        } else {
          char *escaped_value = html_escape(value_to_append);
          if (escaped_value) {
            append_to_buffer(&rendered_buffer, &current_len, &max_len, escaped_value);
            free(escaped_value);
          } else {
            fprintf(stderr,
                    "[ERROR] render_template_segment: Failed to HTML escape value for key '%s'. Appending raw.\n",
                    key_name);
            append_to_buffer(&rendered_buffer, &current_len, &max_len, value_to_append);
          }
        }
...
char *render_template(const char *template_file, TemplateContext *ctx) {
  ...
  if (fp == NULL) {
    ...
    fprintf(stderr, "[ERROR] render_template: Could not open template '%s'. %s\n",
  ...
  if (template_content == NULL) {
    ...
    fprintf(stderr,
            "[ERROR] render_template: Failed to allocate %ld bytes for template content.\n",
            file_size + 1);
    fclose(fp); 
    return NULL;
  }
